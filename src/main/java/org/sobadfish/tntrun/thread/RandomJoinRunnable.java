...
import org.sobadfish.tntrun.manager.PlayerHasChoseRoomManager;
import org.sobadfish.tntrun.manager.RandomJoinManager;
import org.sobadfish.tntrun.manager.ThreadManager;
...
public class RandomJoinRunnable extends ThreadManager.AbstractBedWarRunnable {
    ...
    public boolean joinRandomRoom(RandomJoinManager.IPlayerInfo i){
        ...
        if (i.name == null) {
            ...
            if (roomConfigs.size() > 0) {
                ...
                if (TotalManager.getRoomManager().hasGameRoom(roomConfig.name)) {
                    GameRoom fg = TotalManager.getRoomManager().getRoom(roomConfig.name);
            ...
            if(TotalManager.getMenuRoomManager().worldRoomLinkedHashMap.containsKey(i.name)){
                WorldRoom worldRoom = TotalManager.getMenuRoomManager().getRoom(i.name);
    ...
    private boolean startGameRoom(PlayerInfo info, PlayerHasChoseRoomManager roomManager, GameRoomConfig roomConfig) {
        ...
        if(TotalManager.getRoomManager().enableRoom(TotalManager.getRoomManager().getRoomConfig(roomConfig.name))){
            if (TotalManager.getRoomManager().getRoom(roomConfig.name).joinPlayerInfo(info, true) == GameRoom.JoinType.CAN_JOIN) {
                lock.remove(roomManager);
                return true;
            } else {
                lock.remove(roomManager);
